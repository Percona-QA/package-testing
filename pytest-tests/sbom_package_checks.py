...
One file per product, not one for both: junit names come from the filename, and
the pipelines select results by it (--only test_pxb_sbom). One implementation,
not a copy per product: the package and docker entrypoints have already drifted
apart twice when their logic was duplicated.
...
SBOM_EXTERNAL_TOOLS is on. The molecule jobs install both tools on the target
and turn it on (tasks/install_sbom_tools.yml, tasks/check_pxb_sbom.yml); a
plain local run leaves it off, so those two tests skip.
"""
...
# Exactly what an entrypoint's `import *` picks up: the fixture and the tests.
# Explicit, so importing this module can never overwrite an entrypoint's PRODUCT.
__all__ = [
    "sbom",
...
def _product(request):
    ...
    key = getattr(request.module, "PRODUCT", None)
    if not key:
        pytest.fail("%s does not set PRODUCT; an SBOM entrypoint must bind its "
                    "product, e.g. PRODUCT = \"pxb\"" % request.module.__name__)
    return config.product(key)
