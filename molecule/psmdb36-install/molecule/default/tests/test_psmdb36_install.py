...
def test_package_script(host):
    with host.sudo():
        result = host.run("/package-testing/package_check.sh psmdb36")
        print(result.stdout)
        print(result.stderr)
...
def test_version_script(host):
    with host.sudo():
        result = host.run("command: /package-testing/version_check.sh psmdb36")
        print(result.stdout)
        print(result.stderr)
    assert result.rc == 0, result.stderr
...
def test_decrypt(host, encryption):
    with host.sudo():
        result = host.run("/package-testing/scripts/psmdb_encryption/perconadecrypt-test.sh 3.6 {}".format(encryption))
        print(result.stdout)
        print(result.stderr)
    assert result.rc == 0, result.stderr
...
def test_functional(host):
    with host.sudo():
        result = host.run("/package-testing/scripts/psmdb_test.sh 3.6")
    assert result.rc == 0, result.stderr
