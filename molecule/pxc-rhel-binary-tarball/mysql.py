...
class MySQL:
    def __init__(self, base_dir):
        ...
        subprocess.call(['chown', '-R', 'mysql:mysql', self.basedir + '/log'])
        ...
        if self.major_version.startswith("5."):
            self.sst_opts = ["--wsrep_sst_method=xtrabackup-v2", "--wsrep_sst_auth=root:"]
        else:
            self.sst_opts = ["--wsrep_sst_method=xtrabackup-v2", "--user=mysql"]
        if self.major_version == "5.6":
            ...
            subprocess.check_call([self.mysqld, '--no-defaults', '--user=mysql', '--initialize-insecure', '--basedir=' + self.basedir,
                                   '--datadir=' + self.node1_datadir])
            subprocess.check_call([self.mysqld, '--no-defaults', '--user=mysql', '--initialize-insecure', '--basedir=' + self.basedir,
                                   '--datadir=' + self.node2_datadir])
            subprocess.check_call([self.mysqld, '--no-defaults', '--user=mysql', '--initialize-insecure', '--basedir=' + self.basedir,
