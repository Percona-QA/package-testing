...
checkec2-qa(){
           ...
           aws ec2 describe-instances --filters "Name=instance-state-name,Values=running" "Name=tag:job-name,Values=*pxb*,*package-testing*,*ps*,*pxc*,*pbm*,*pdmdb*,*orchestrator_docker*,*toolkit*,*pt*" --query "Reservations[].Instances[?LaunchTime<='${threshold_date}'].[Tags[?Key=='Name'].Value[],Tags[?Key=='job-name'].Value[], InstanceId, LaunchTime]" --output yaml --region $1 | sed -e 's/\- \[\]//g' -e '/^$/d'
