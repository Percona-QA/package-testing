...
# use perconalab or percona
DOCKER_HUB="percona"
PXC_VERSION="5.7"

# use global versions file for version check
# source ../VERSIONS

#if [ ${PXC_VERSION} == "5.6" ]; then
#  PXC_TAG="5.6"
#  version=${PXC56_VER%-*}
#  revision=${PXC56_REV}
#  wsrep=${PXC56_WSREP}
...
#  PXC_TAG="5.7"
#  SERVER_VERSION=${PXC57_VER%-*}
#  SERVER_REVISION=${PXC57_REV}
#  WSREP_VERSION=${PXC57_WSREP}
#fi

# version check
PXC_TAG="5.7.19"
SERVER_VERSION="5.7.19-17"
SERVER_REVISION="c10027a"
WSREP_VERSION="3.22(r8678538)"
...
@test "start pxc node 1" {
  run bash -c "docker run -d -p 3306 --name=pxc_node1 --net=${CLUSTER_NAME}_net -e DISCOVERY_SERVICE=${ETCD_HOST}:2379 -e MYSQL_ROOT_PASSWORD=Theistareyk -e XTRABACKUP_PASSWORD=Theistare -e CLUSTER_NAME=${CLUSTER_NAME} ${DOCKER_HUB}\/percona-xtradb-cluster:${PXC_TAG}"
  [ $status -eq 0 ]
}

@test "start pxc node 2" {
  run bash -c "docker run -d -p 3306 --name=pxc_node2 --net=${CLUSTER_NAME}_net -e DISCOVERY_SERVICE=${ETCD_HOST}:2379 -e MYSQL_ROOT_PASSWORD=Theistareyk -e XTRABACKUP_PASSWORD=Theistare -e CLUSTER_NAME=${CLUSTER_NAME} ${DOCKER_HUB}\/percona-xtradb-cluster:${PXC_TAG}"
  [ $status -eq 0 ]
}

@test "start pxc node 3" {
  run bash -c "docker run -d -p 3306 --name=pxc_node3 --net=${CLUSTER_NAME}_net -e DISCOVERY_SERVICE=${ETCD_HOST}:2379 -e MYSQL_ROOT_PASSWORD=Theistareyk -e XTRABACKUP_PASSWORD=Theistare -e CLUSTER_NAME=${CLUSTER_NAME} ${DOCKER_HUB}\/percona-xtradb-cluster:${PXC_TAG}"
  [ $status -eq 0 ]
}

@test "start proxysql" {
  run bash -c "docker run -d -p 3306:3306 -p 6032:6032 --net=${CLUSTER_NAME}_net --name=${CLUSTER_NAME}_proxysql -e DISCOVERY_SERVICE=${ETCD_HOST}:2379 -e CLUSTER_NAME=${CLUSTER_NAME} -e ETCD_HOST=${ETCD_HOST} -e MYSQL_ROOT_PASSWORD=Theistareyk -e MYSQL_PROXY_USER=proxyuser -e MYSQL_PROXY_PASSWORD=s3cret ${DOCKER_HUB}\/proxysql"
  [ $status -eq 0 ]
}
...
@test "check PXC server version" {
  sleep 100
  result=$(docker exec -ti pxc_node1 mysql -uroot -pTheistareyk -e "SHOW VARIABLES LIKE 'version';" | grep -c ${SERVER_VERSION})
  [ $result -eq 1 ]
}

@test "check PXC innodb version" {
  result=$(docker exec -ti pxc_node1 mysql -uroot -pTheistareyk -e "SHOW VARIABLES LIKE 'innodb_version';" | grep -c ${SERVER_VERSION})
  [ $result -eq 1 ]
}

@test "check PXC comment version" {
  result=$(docker exec -ti pxc_node1 mysql -uroot -pTheistareyk -e "SHOW VARIABLES LIKE 'version_comment';" | grep -c ${SERVER_REVISION})
  [ $result -eq 1 ]
}

@test "check PXC wsrep version" {
  result=$(docker exec -ti pxc_node1 mysql -uroot -pTheistareyk -e "SHOW STATUS LIKE 'wsrep_provider_version';" | grep -c ${WSREP_VERSION})
  [ $result -eq 1 ]
}
